...
Gearman.idCounter = 0;
Gearman.generateId = function(){
    return "G:" + Gearman.idCounter++;
}
...
Gearman.prototype.connect = function(){
    ...
    this.socket = netlib.createConnection(this.port, this.host);
...
Gearman.prototype.sendCommandToServer = function(){
    ...
    if(args.length && typeof args[args.length-1] == "function"){
        ...
        this.callbackQueue.push(commandCallback);
    }
...
Gearman.prototype.receive = function(chunk){
    ...
    if(typeof this["receive_" + commandName] == "function"){
        if(this.callbackQueue.length){
            args = args.concat(this.callbackQueue.shift());
        }
...
Gearman.prototype.receive_WORK_FAIL = function(handle){
    if(this.currentJobs[handle]){
        this.currentJobs[handle].emit("error", new Error("Job failed"));
...
Gearman.prototype.receive_WORK_COMPLETE = function(handle, payload){
    if(this.currentJobs[handle]){
        this.currentJobs[handle].emit("data", payload);
        this.currentJobs[handle].emit("end");
        delete this.currentJobs[handle];
    }
}
...
Gearman.prototype.submitJob = function(name, payload, callback){
    return new this.Job(this, name, payload, callback);
}
...
Gearman.prototype.Job = function(gearman, name, payload, callback){
    EventEmitter.call(this);
...
utillib.inherits(Gearman.prototype.Job, EventEmitter);
